...
def verifier(relation_filepath, iter_num):
    ...
    hard_matched.to_csv(relation_filepath + '6_verify_result/' + str(iter_num) + '.csv', columns=['entity1', 'entity2', 'sent', 'processed_sent', 'human'])

    hard_unmatched = hard_match_result[hard_match_result['hard_match'] == 0]
    hard_unmatched.to_csv(relation_filepath + '4_sent_to_match/' + str(iter_num + 1) + '.csv', columns=['entity1', 'entity2', 'sent', 'processed_sent', 'label'])
