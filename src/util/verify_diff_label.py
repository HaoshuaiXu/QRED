...
def verifier(relation_filepath, iter_num, label_prop=1):
...
def filter(hard_df:pd.DataFrame, soft_df:pd.DataFrame, label_prop):
    if int(label_prop) == 1:
        ...
        hard_unsample = hard_df.copy()
        hard_unsample = hard_unsample.append(hard_match_conflict)
        hard_unsample.drop_duplicates(keep=False)

        soft_match_same = soft_df[soft_df['soft_match'] == soft_df['model']].copy()
        soft_match_conflict = soft_df[soft_df['soft_match'] != soft_df['model']].copy().sample(frac=label_prop)
        soft_unsample = soft_df.copy()
        soft_unsample = soft_unsample.append(hard_match_conflict)
        soft_unsample.drop_duplicates(keep=False)
